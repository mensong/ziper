...
std::string CDirEntry::dirName(const std::string& path)
{
    ...
    //further deal if find / in the end of path above
...
{
    if (isRelativePath(absolutePath) || isRelativePath(relativeTo)) return false; // Nothing can be done.
...
{
    if (!isRelativePath(relativePath) || isRelativePath(absoluteTo)) return false; // Nothing can be done.
