  ...
  public:
    ...
    void changeFileDate(const std::string& filename, uLong dosdate, tm_unz tmu_date)
    {
      ...
      if (hFile != INVALID_HANDLE_VALUE)
      {
        ...
        DosDateTimeToFileTime((WORD)((dosdate >> 16)), (WORD)(dosdate & 0xFFFF), &ftLocal);
    ...
    int extractToStream(std::ostream& stream, ZipEntry& info)
    {
      ...
      do
      {
        err = unzReadCurrentFile(m_zf, &buffer[0], (unsigned int)buffer.size());
		if (err < 0 || err == 0)
			break;

        stream.write(&buffer[0], err);
        if (!stream.good())
        {
          err = UNZ_ERRNO;
          break;
    ...
    int extractToMemory(std::vector<unsigned char>& outvec, ZipEntry& info)
    {
      ...
      do
      {
        ...
        outvec.insert(outvec.end(), &buffer[0], &buffer[0] + read);
  ...
  public:
    ...
    bool extractAll(const std::string& destination, const std::map<std::string, std::string>& alternativeNames)
    {
      ...
      for (; it != entries.end(); ++it)
      {
		...
		std::map<std::string, std::string>::const_iterator itNames = alternativeNames.find(it->name);
        if (itNames != alternativeNames.end())
          alternativeName += itNames->second;
        else
