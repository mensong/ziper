...
namespace zipper {
	...
	struct Zipper::Impl
	{
		...
		bool initFile(const std::string& filename, zipFlags flags)
		{
			#ifdef USEWIN32IOAPI
		...
		bool initWithVector(std::vector<unsigned char>& buffer)
		{
			...
			if (!buffer.empty())
			{
				...
				memcpy(m_zipmem.base, (char*) &buffer[0], buffer.size());
		...
		bool add(std::istream& input_stream, const std::string& nameInZip, const std::string& password, int flags)
		{
			...
			if (ZIP_OK == err)
			{
				do {
					err = ZIP_OK;
					input_stream.read(&buff[0], buff.size());
					size_read = (size_t)input_stream.gcount();
					if (size_read < buff.size() && !input_stream.eof() && !input_stream.good())
						err = ZIP_ERRNO;

					if (size_read > 0)
						err = zipWriteInFileInZip(this->m_zf, &buff[0], (unsigned int)size_read);
	...
	Zipper::Zipper(const std::string& zipname, zipFlags flags/* = Overwrite*/)
		: m_obuffer(*(new std::stringstream())) //not used but using local variable throws exception
		, m_vecbuffer(*(new std::vector<unsigned char>())) //not used but using local variable throws exception
		, m_usingMemoryVector(false)
		, m_usingStream(false)
	...
	Zipper::Zipper(const std::string& zipname, const std::string& password, zipFlags flags/* = Overwrite*/)
		: m_obuffer(*(new std::stringstream())) //not used but using local variable throws exception
		, m_vecbuffer(*(new std::vector<unsigned char>())) //not used but using local variable throws exception
		, m_usingMemoryVector(false)
		, m_usingStream(false)
	...
	bool Zipper::openZipFileName(const std::string& zipFileName, zipFlags flags/* = Overwrite*/)
	{
		close();

		if (!m_impl->initFile(zipFileName, flags))
		{
			return false;
		}
		m_open = true;
	...
	void Zipper::open()
	{
		if (!m_open)
		{
			...
			{
				if (!m_impl->initFile(m_zipname, Overwrite))
