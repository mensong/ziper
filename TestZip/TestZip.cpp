...
{
	{
		...
		z->addFolder("tmp//", NULL, IZip::Faster);
		z->add("..\\//zipper.sln", "sln1\\zipper.sln", IZip::Faster);
		z->reopen(IZip::Append);
		z->addFolder("tmp//", "tmp2\\/", IZip::Faster);
		z->add("..\\//zipper.sln", "sln2\\zipper.sln", IZip::Faster);
		z->close();
		zip::Ins().ReleaseZip(z);
	}
